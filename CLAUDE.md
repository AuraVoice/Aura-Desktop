...
Meeting Notes (MEETING_NOTES_PLAN.md) adds `src-tauri/src/meeting/` (WASAPI capture, join detection, encrypted segment queue) plus `useMeetingPrompt`/`useMeetingCapture`/`useMeetingNotes` on the React side; capture is user-armed only, shows a persistent recording indicator, and gates the updater like `voice_active` does. Arming happens at the moment of the call: `detect.rs`'s ambient watch (one thread per process, calendar or not) emits `meeting-call-seen`/`meeting-call-gone`, and `useMeetingPrompt` shows the "Record this meeting?" card (`MeetingPromptCard`) in the slot via `summon_bar`, never `summon`. Nothing records without Record or the tray's Capture now; the old silent `automaticCapture` path and the global auto-notes toggle are gone.
