...
//! an edit-distance ratio, no model call. It runs on the observer thread, never
//! on the dictation path, but the bigger reason is that a training corpus built
//! by an opaque judgement is a training corpus nobody can audit.
//!
//! Restored from the pre-`9227b79` `trace/diff.rs` with the edit types folded
//! in. The five classes and the JSON shape (`class`, `from`, `to`, `wordIndex`)
//! are the backend's `EditPayload`, so what is stored on the history row is
//! what the upload sends.
...
/// How one aligned block of the diff is classified. Mirrors the backend's
/// `EditPayload.class` literal exactly.
...
impl EditClass {
    /// Whether this class belongs in the ground-truth transcript. Verbatim,
    /// casing and punctuation all describe the recognizer mis-rendering audio;
    /// the other two describe the user changing what they wanted to say.
...
/// One classified change. Serialized as the backend's `EditPayload`.
...
pub struct EditOp {
    ...
    pub to: String,
    /// Word offset into the inserted text.
...
/// what lets a rebuilt string preserve the original spacing exactly.
#[derive(Clone, Debug)]
struct Token {
    lead: String,
    text: String,
...
pub struct Comparison {
    ...
    /// uploaded as `trainingText`: for a clean correction it is the user's
    /// fix, for a rewrite it is what Aura typed.
...
pub fn compare(inserted: &str, final_text: &str) -> Comparison {
    ...
    // record of what changed - but hand back what Aura typed, so nothing
    // downstream can mistake a stitched string for a transcript of the audio.
...
/// Double-metaphone equality across the whole block.
