...
//!
//! 1. **Nothing is parked unless sharing is switched on.** The caller passes
//!    that decision in (`share::sharing_hint`); this module never assumes. The
//!    hold-context read (role, app, caret prefix for the formatter) is the one
//!    read that happens regardless, and it never crosses to disk or the network
//!    except through `/dictation/polish`, which is `no-store`.
...
/// A whole field this short IS its own caret prefix for register purposes
/// (a subject line, a chat composer, a search box). Longer fields only yield a
/// prefix through TextPattern, which knows where the caret actually is.
const SHORT_FIELD_PREFIX_CHARS: usize = 200;
...
/// A baseline is parked when the chord goes DOWN (so nothing new sits between
/// the recognizer's final and `SendInput`), and a hold can last up to
/// `dictation::audio::MAX_HOLD` (120 s). Past that plus a margin the field has
/// had time to change underneath us and the "before" no longer describes what
/// was typed into. The user cannot type while holding the chord, which is what
/// makes a baseline this old still trustworthy.
const PENDING_BASELINE_TTL: Duration = Duration::from_secs(130);
...
/// What one hold-start round trip learned about the focused field, for the
/// formatter and the stored trace context. Every field is optional: a
/// password field, a window of Aura's own, a control with no text pattern or
/// a password manager each yield less, never an error.
#[derive(Clone, Debug, Default)]
pub struct HoldContext {
    /// UI Automation control type name ("Edit", "Document", "ComboBox").
    pub role: Option<String>,
    /// Executable stem of the owning process, lowercased.
    pub app: Option<String>,
    /// Up to `prefix_chars` characters immediately before the caret.
    pub prefix: Option<String>,
    /// Whether a baseline was parked for the insert that follows.
...
impl AnchorStore {
    ...
    /// Called when the chord goes down, before the HUD exists. Reads the
    /// focused field's role, app and caret prefix, and, when `park` is set,
    /// parks its "before" text so the insert that follows can be verified.
    ///
    /// Sensitive fields yield nothing at all, not even the role: the caller
    /// cannot tell "no field" from "a field we refused to look at", which is
    /// the point.
    pub fn hold_context(
        &mut self,
        ...
        if is_own_process(&focused) || is_sensitive_element(&focused) {
            return HoldContext::default();
        }
        let identity = identity_of(&focused);
        if identity
            .as_ref()
            .is_some_and(|identity| SENSITIVE_APPS.contains(&identity.app.as_str()))
        {
            return HoldContext::default();
        }
        let prefix = caret_prefix(&focused, prefix_chars);
        if park {
            self.park_baseline(&focused);
        }
        HoldContext {
            role: identity.as_ref().map(|identity| identity.role.clone()),
            app: identity.map(|identity| identity.app),
            prefix,
            baseline_parked: self.pending.is_some(),
        }
    }
...
/// The text immediately before the caret, bounded.
///
/// TextPattern first: it knows where the caret is, so the window is exact.
/// ValuePattern only for a field short enough that the whole value is the
/// prefix; a long value with no caret position would be a guess dressed up as
/// context, and the formatter is better off with nothing.
fn caret_prefix(element: &IUIAutomationElement, chars: usize) -> Option<String> {
    if chars == 0 {
        return None;
    }
    if let Ok(pattern) =
        ...
        if let Ok(selection) = unsafe { pattern.GetSelection() } {
            if unsafe { selection.Length() }.unwrap_or(0) >= 1 {
                if let Ok(range) = unsafe { selection.GetElement(0) } {
                    if let Ok(window) = unsafe { range.Clone() } {
                        // Collapse to the caret, then walk the start back.
                        let _ = unsafe {
                        ...
                        if let Ok(text) = unsafe { window.GetText(chars as i32) } {
    ...
    if count == 0 || count > SHORT_FIELD_PREFIX_CHARS.min(chars) {
        return None;
    }
    Some(value)
}
